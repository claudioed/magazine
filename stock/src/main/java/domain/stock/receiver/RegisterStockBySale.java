...
public class RegisterStockBySale extends AbstractVerticle {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(RegisterStockBySale.class);
    ...
    public void start() throws Exception {
        ...
        eb.consumer(DomainEvent.REGISTER_ITEM_BY_SALE.event(), message -> {
            ...
            mongoClient.updateWithOptions(DomainCollection.ITEMS.collection(),query,update,options,result ->{
                if(result.failed()){
                    LOGGER.error("Error on update stock by sale !!! Any rows affected !!!");
                } else {
                    LOGGER.info("Success on update stock by sale !!!");
                }
            });
        });
    }
